...
void CL_InitRef( void )
{
	...
	//ri.IN_Init = IN_Init; // Cowcat
	ri.IN_Shutdown = IN_Shutdown;
	ri.IN_Restart = IN_Restart;

	ri.ftol = Q_ftol;

	//ri.Sys_SetEnv = Sys_SetEnv; // Cowcat
...
void CL_startMultiplayer_f( void )
{
	...
	//Sys_StartProcess( binName, qtrue ); // Cowcat
#endif
}
// -NERVE - SMF
...
void CL_ShellExecute_URL_f( void ) {
	...
	//Sys_OpenURL( Cmd_Argv( 2 ),doexit ); // Cowcat
}
//----(SA) end
