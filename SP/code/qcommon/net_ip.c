...
#	if !defined(__sun) && !defined(__sgi) && !defined(AMIGAOS) && !defined(MORPHOS)
#		include <ifaddrs.h>
#	endif
...
qboolean NET_GetPacket( netadr_t *net_from, msg_t *net_message, fd_set *fdr )
{
	...
	if(ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr))
	{
		...
		#if defined(MORPHOS)
		ret = recvfrom( ip_socket, (void *)net_message->data, net_message->maxsize, 0, (struct sockaddr *) &from, (long *)&fromlen );
		#else
		...
		if (ret == SOCKET_ERROR)
		{
			...
			#if !defined(MORPHOS) // always spam - Cowcat
...
#if defined(AMIGAOS) || defined(MORPHOS)// AmigaOS uses IP4
...
#if defined (DEDICATED) || defined(AMIGAOS) || defined(MORPHOS)
