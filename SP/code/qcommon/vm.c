...
void VM_Init( void )
{
	...
	#else
	Cvar_Get( "vm_cgame", "2", CVAR_ARCHIVE );	// !@# SHIP WITH SET TO 0
	Cvar_Get( "vm_game", "2", CVAR_ARCHIVE );	// !@# SHIP WITH SET TO 0
	Cvar_Get( "vm_ui", "2", CVAR_ARCHIVE );		// !@# SHIP WITH SET TO 0
	#endif
...
{
	vm_t		*vm;
	vmHeader_t	*header; // = NULL; // null Cowcat
	...
	do
	{
		...
		if(retval == VMI_NATIVE)
		{
			...
			//vm->dllHandle = Sys_LoadDll(filename, vm->fqpath, &vm->entryPoint, VM_DllSyscall); // Cowcat
