...
void RB_TakeScreenshotJPEG(int x, int y, int width, int height, char *fileName)
{
	...
	//RE_SaveJPG(fileName, r_screenshotJpegQuality->integer, width, height, buffer + offset, padlen); // Cowcat
...
//----(SA)	end
	...
	r_ext_texture_env_add = ri.Cvar_Get( "r_ext_texture_env_add", "0", CVAR_ARCHIVE | CVAR_LATCH ); // Cowcat
...
//----(SA)	end
	...
	// Rafael - wolf fog
	r_wolffog = ri.Cvar_Get( "r_wolffog", "0", 0 ); // was 1 - Cowcat
	// done
	...
	r_shadows = ri.Cvar_Get( "cg_shadows", "0", 0 ); // Cowcat
